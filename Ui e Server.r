...
library(shinythemes)
library(htmltools)
ristoranti <- read_excel("C://Users//Susanna//Desktop//Dove Andiamo a Mangiare//AnonymousExcel.xlsx")
str(ristoranti)
...
ui<-fluidPage( 
  ...
  sidebarLayout(      
    ...
    mainPanel(
      tabsetPanel(type = "tabs", #type tabs
                  tabPanel("Grafico e Graduatoria",
                           br(),
                           fluidRow(plotlyOutput("phonePlot",height = 450,width = 750)),
...
server<-function(input, output) {
  ...
  output$t<-renderTable({
   ...
   tabella=data.frame(ciao=l,dario=rbind(t(ristoranti[ristoranti$Nome==input$risto,c("Nome","Prezzo","Recensioni","Cucina10","Servizio10","Atmosfera10","QualitaPrezzo1","Rumorosita1","TempoAttesa1")]),
                                         round(t(ristoranti[ristoranti$Nome==input$risto,c("Sentiment","Indice")]),5)))
   colnames(tabella)=c("","  ")
   tabella
  })
  ...
  output$t1<-renderTable({
    ...
    tabella=data.frame(ciao=l,dario=rbind(t(ristoranti[ristoranti$Nome==input$risto1,c("Nome","Prezzo","Recensioni","Cucina10","Servizio10","Atmosfera10","QualitaPrezzo1","Rumorosita1","TempoAttesa1")]),
                                          round(t(ristoranti[ristoranti$Nome==input$risto1,c("Sentiment","Indice")]),5)))
    colnames(tabella)=c("","  ")
    tabella
    }
  })
  ...
  output$phonePlot <- renderPlotly({
    ...
    plot_ly(ristoranti1,x=ristoranti1$Votofinale,y=ristoranti1$Indice,text=ristoranti1$Nome,mode="markers",
            ...
            , symbol=ristoranti1$Quartiere,symbols=rep("octagon",10) 
                         ...
                         legend=leg)
    #exagon-open, circle-open, octagon
   # "blueviolet","lightslateblue",
   # "turquoise1","navy",
    #"darkgreen","chartreuse2",
    #"yellow2","orangered",
    #"red","red4"
  }
  )
  ...
  output$mymap=renderLeaflet({
    ...
    leaflet() %>%
     ...
     # addPopups(lng=ristoranti1$Longitudine, lat=ristoranti1$Latitudine, paste(sep = "<br/>","<b><a href='https://www.thefork.it/ristorante/abbottega/218491'>", ristoranti1$Nome , "</a></b>",
            #                                                                   ristoranti1$Indirizzo
            #                                                                   ),options = popupOptions(closeButton = FALSE)
     #)
      addMarkers(lng=ristoranti1$Longitudine, lat=ristoranti1$Latitudine, label=ristoranti1$Nome) #,popup = c(ristoranti1$Nome))
